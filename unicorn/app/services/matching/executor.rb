...
module Matching
  class Executor
    ...
    def validate!
      ...
      if ask.ord_type == 'limit' && ask.price > @price
        raise_error(3001, 'Ask price exceeds strike price.')
      end
      if bid.ord_type == 'limit' && bid.price < @price
        raise_error(3002, 'Bid price is less than strike price.')
      end
      unless @maker_order.state == Order::WAIT
        raise_error(3003, "Maker order state isn\'t equal to «wait» (#{@maker_order.state}).")
      end
      unless @taker_order.state == Order::WAIT
        raise_error(3004, "Taker order state isn\'t equal to «wait» (#{@taker_order.state}).")
      end
